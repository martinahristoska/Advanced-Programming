...
class GenericCollection<T extends Comparable<T> & IHasTimestamp>
{
    ...
    TreeMap<LocalDateTime,T> mapByLocalDateTime;

    public GenericCollection()
    {
        this.mapByCategory = new HashMap<>();
        this.mapByLocalDateTime = new TreeMap<>();
    }
    ...
    public void addGenericItem(String category, T element) {
        ...
        mapByLocalDateTime.put(element.getTimestamp(),element);
    }

    public Collection<T> itemsFromCategories(List<String> categories) {
        TreeSet<T> treeSet =  categories.stream()
                .flatMap(c->mapByCategory.get(c).stream())
                .collect(Collectors.toCollection(TreeSet::new));
        TreeSet<T> finalSet = new TreeSet<>(Comparator.reverseOrder());
        finalSet.addAll(treeSet);
        return finalSet;
    }

    public Collection<T> findAllBetween(LocalDateTime from, LocalDateTime to) {
        TreeSet<T> treeSet = new TreeSet<>(mapByLocalDateTime.subMap(from, to).values());
        TreeSet<T> finalSet = new TreeSet<>(Comparator.reverseOrder());
        finalSet.addAll(treeSet);
        return finalSet;
    }
    ...
    public Map<String,Set<T>> byMonthAndDay() {
        return mapByLocalDateTime.entrySet()
                .stream()
                .collect(Collectors.toMap(
                        entry->String.format("%02d-%02d",entry.getKey().getMonthValue(),entry.getKey().getDayOfMonth()),
                        entry->{
