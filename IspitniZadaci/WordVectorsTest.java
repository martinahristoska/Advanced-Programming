...
class WordVectors
{
    ...
    final Comparator<Word> wordComparator = Comparator.comparing(l->l.word);

    public WordVectors(String [] words, List<List<Integer>> vectors)
    {
        this.wordVectorMap = new TreeMap<>(wordComparator);
    ...
    public List<Integer> slidingWindow(int n) {
        return IntStream.range(0,vectors.size() - n + 1)
                .mapToObj(i -> vectors.subList(i,i+n).stream()
