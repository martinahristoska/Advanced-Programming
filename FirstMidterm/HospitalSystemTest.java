...
class HospitalSystem<T extends Patient & Comparable<T>>
{
    ...
    public void printResults() {
        PatientService<T> doctorWithNameFilter = (patientsList,filter) ->
                patientsList.stream()
                .filter(d->d.getDoctor().equals(filter))
                .collect(Collectors.toCollection(ArrayList::new));

        PatientService<T> notOlderAndGender = (patientsList, filter) ->
                patientsList.stream()
                .filter(p->p.getAge()<=60 && p.getGender().equals(filter))
                .collect(Collectors.toCollection(ArrayList::new));

        PatientService<T> nameFilter = (patientsList,filter) ->
                patientsList.stream()
                .filter(p->p.getName().equals(filter))
                .sorted()
                .collect(Collectors.toCollection(ArrayList::new));

        System.out.println("FIRST SERVICE INFORMATION");
        doctorWithNameFilter.patientsInformation(patients,"Peter").forEach(System.out::println);
        System.out.println("SECOND SERVICE INFORMATION");
        notOlderAndGender.patientsInformation(patients,"FEMALE").forEach(System.out::println);
        System.out.println("THIRD SERVICE INFORMATION");
        nameFilter.patientsInformation(patients,"Sarah").forEach(System.out::println);
    }
}
