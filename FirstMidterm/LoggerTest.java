...
class LogSystem<T extends ILog & Comparable<T>> {
    ...
    void printResults() {
        ...
        LogProcessor<T> firstLogProcessor =
                (logs) -> logs.stream().filter(l->l.getType().equals("INFO"))
                .collect(Collectors.toCollection(ArrayList::new));

        LogProcessor<T> secondLogProcessor =
                (logs) -> logs.stream().filter(l->l.getMessage().length()<100)
                .collect(Collectors.toCollection(ArrayList::new));

        LogProcessor<T> thirdLogProcessor =
                (logs) -> logs.stream().sorted()
                .collect(Collectors.toCollection(ArrayList::new));
